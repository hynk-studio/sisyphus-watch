...
test("public live XHTML owner is capture-only while target identity remains independently supported", async () => {
  const admission = new FakeAdmissionStore();
  const fixture = publicLiveInternalFixture();
  let internalRuns = 0;
  let providerCalls = 0;
  ...
  const response = await handlePublicLiveLineageRequest(
    ...
    }),
    {
      ...
      capture: {
        ...
        fetcher: (async (input) => {
          captureCalls += 1;
          const isOwner = String(input).includes("source-owner");
          return new Response(
            isOwner
              ? [
  ...
  assert.equal(captureCalls, 2);
  assert.equal(observedInternals.length, 1);
  const internal = observedInternals[0];
  assert.equal(internal.documents.length, 2);
  assert.equal(
    internal.documents.every((item) => item.capture_completeness === "complete"),
    true,
  );
  assert.equal(internal.supports.length, 0);
  assert.equal(internal.source_supported_target_identity_proofs.length, 1);
  assert.equal(internal.source_supported_relation_assessments.length, 0);
  assert.equal(
    internal.source_supported_relation_work_summary.rejected_capture_support_count,
    1,
  );
  assert.equal(
  ...
  assert.doesNotMatch(
    JSON.stringify(body),
    /source_supported_target_identity|target_identity_proof_id|document_identity|identity_anchor|captured_document_self_identity_matches_resolved_target_metadata|captured_body_sha256|normalized_text_sha256|source_supported_relation_assessment|This guidance supersedes Guidance G-1/u,
  );
});
